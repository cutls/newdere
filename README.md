...
↑2023年2月4日現在 **ボイス実装済み** | [ボイス未実装(2023/2/15)](https://github.com/cutls/newdere/blob/main/res/2023-02-15-nocv.png) | [限定スキル秒数対応表(2023/2)](https://github.com/cutls/newdere/blob/main/res/2023-02-04-limited.png) | [ブランスキル秒数対応表(2023/1/30)](https://github.com/cutls/newdere/blob/main/res/2023-01-30-blane.png)

スキル対応表が必要ない恒常やノワールでは寂しいので新規カードの画像を投稿します。

![剥きだしのココロ 小日向美穂](/res/chara.png)  
...
* 新スキルが来たら？  
オーバードライブの時は盛大に動作不能状態を作り出しました。現在(2023/2/15)利用APIではオーバードライブに対応していないので、未定義のスキルがあった場合は自動的にオーバードライブ扱いになるような処理をしています。なお、キャラクター画像はスキル表が用いられる限定ガシャでは投稿されないため、もしキャラクター画像作成時に未定義のスキルがあった場合は、スキルの説明を画像に挿入します。
...
1. (限定/ブランの場合)スキル秒数対応表を作成します。`limited.json`または`blane.json`を以上の操作から得た更新情報を元に書き足します。
1. (恒常/ノワールの場合)キャラ画像を作成します。
1. 画像をツイートします。
